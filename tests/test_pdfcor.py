...
from pdfcor.utils import slugify, resize_for_a4
from pdfcor.cli import app as cli_app # Changed from main to app
...
class TestPdfCor(unittest.TestCase):
    ...
    def _create_dummy_pdf(self, filepath: Path, text_contents: list[str] | str): # Allow Path object
        doc = fitz.open()
    ...
    def _create_dummy_image(self, filepath: Path, size=(100, 100), color="blue"): # Allow Path object
        img = Image.new('RGB', size, color=color)
        img.save(filepath) # PIL Image.save can handle Path objects

    def _create_pdf_with_image(self, pdf_filepath: Path, image_filepath: Path, image_rect=fitz.Rect(50, 100, 150, 200)): # Allow Path
        doc = fitz.open()
        page = doc.new_page()
        try:
            ...
            print(f"Warning: page.insert_image with filename failed ({e}), trying stream.")
    ...
    def test_process_pdf_extraction(self):
        ...
        md_output_dir.mkdir(parents=True, exist_ok=True) # process_pdf expects output_dir to exist
    ...
    def test_merge_pdfs_basic(self):
        ...
        merge_pdfs(self.test_input_path, output_file=merged_output_filename, output_dir=self.test_output_path)
    ...
    def test_process_pdf_with_image_extraction(self):
        ...
        found_images = [f.name for f in expected_img_dir.iterdir() if f.name.startswith(slug_pdf_name) and (f.name.endswith(".png") or f.name.endswith(".jpg"))]
    ...
    def test_process_pdf_with_image_resize(self):
        ...
        found_images_resized = [f for f in expected_img_dir.iterdir() if f.name.startswith(slug_pdf_name)]
    ...
    def test_cli_process_default_output(self): # Renamed
        ...
        self._create_dummy_pdf(pdf_on_input_path, "CLI Test Content for process default")

        result = self.runner.invoke(cli_app, ["process", "--input-folder", str(abs_input_dir)])

        self.assertEqual(result.exit_code, 0, result.stdout)
        expected_md_file: Path = abs_input_dir / "pdfcor_output" / "cli_doc.md"
        self.assertTrue(expected_md_file.exists(), f"Expected MD file not found at {expected_md_file}\nOutput:\n{result.stdout}")
    ...
    def test_cli_merge_custom_output(self): # Renamed
        ...
        result = self.runner.invoke(cli_app, [
            "merge",
            "--input-folder", str(abs_input_dir),
            "--output-file", str(output_pdf_file_path)
        ])

        self.assertEqual(result.exit_code, 0, result.stdout)
        self.assertTrue(output_pdf_file_path.exists(), f"Fused CLI output PDF not found at {output_pdf_file_path}\nOutput:\n{result.stdout}")
    ...
    def test_cli_process_recursive(self): # Renamed
        ...
        result = self.runner.invoke(cli_app, [
            "process",
            "--input-folder", str(abs_input_dir),
            "--output-folder", str(output_dir_rec),
            "--recursive"
        ])
    ...
    def test_cli_extract(self): # New test
        ...
        self._create_dummy_pdf(multipage_pdf_path, ["CLI Page 1 Text", "CLI Page 2 Text"])
        ...
        result = self.runner.invoke(cli_app, ["extract", resolved_multipage_pdf_path_str])
        ...
        self.assertTrue(expected_pages_dir.exists(), f"Extracted pages directory not found at {expected_pages_dir}\nOutput:\n{result.stdout}")
