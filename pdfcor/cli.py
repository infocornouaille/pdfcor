...
@app.command("process", help="Extracts content from PDFs to Markdown, including images.")
def process_cli(
    ...
    output_folder_opt: Optional[Path] = typer.Option(
        ...
        file_okay=False, dir_okay=True, writable=True, resolve_path=True, # resolve_path for user convenience
    ...
    resize: bool = typer.Option(
        False,
        "--resize",
        help="Resize extracted images to fit A4 page dimensions."
    )
) -> None:
    """
...
def merge_cli(
    ...
    output_file_opt: Optional[Path] = typer.Option( # Changed from output_path_opt to output_file_opt for clarity
        None,
        "--output-file", "-out",
        ...
        writable=True, dir_okay=False, resolve_path=True, # Allow creating the file
    ...
    final_output_name = output_filename_arg if output_filename_arg is not None else f"{slugify(input_folder.name)}.pdf" # Replicate core's default name for echo
